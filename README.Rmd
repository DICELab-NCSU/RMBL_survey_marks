...
pts <- read_sf("current_mark-points.geojson") |>
  ...
  rowwise() |>
  mutate(lon = st_coordinates(geometry)[1, 1],
         lat = st_coordinates(geometry)[1, 2]) |>
  st_drop_geometry() |>
  select(name, type, lon, lat) |>
  ungroup()

m <- leaflet(data = pts) |>
  addProviderTiles(provider = "OpenTopoMap") |>
  addMarkers(lng = ~lon, lat = ~lat, label = ~name)
mapshot2(m, file = "utilities/overview-map.png")
```
![Overview map of all survey mark points.](utilities/overview-map.png)
...
The raster file for the DEM is not provided in this repo due to its large size.

#### 1.2. Calculating viewsheds

The code needed to reproduce the viewsheds is provided in `code/2_mk_viewsheds.R`. The 
